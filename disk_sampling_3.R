...
constellations <- disk_sampling(30, 25, .01) %>%
  mutate(type = "constellation")
...
ggplot(data = df[df$graph == 1, ]) +
  geom_point(aes(x = new_x, y = new_y, color = type)) +
  coord_equal() +
  scale_x_continuous(limits = c(-(pi / 2), (pi / 2))) +
  scale_y_continuous(limits = c(-(pi / 2), (pi / 2)))
ggplot(data = df[df$graph == 2, ]) +
  geom_point(aes(x = new_x, y = new_y, color = type)) +
  coord_equal() +
  scale_x_continuous(limits = c(-(pi / 2), (pi / 2))) +
  scale_y_continuous(limits = c(-(pi / 2), (pi / 2)))

# maybe remove stars that are too close? - maybe not
