...
ggplot(data = sky) +
  geom_segment(data = lines, aes(x =  new_x_1, y = new_y_1, 
                                 xend = new_x_2, yend = new_y_2)) +
  geom_text(data = lines, aes(x = (new_x_1 + new_x_2) / 2,
                              y = (new_y_1 + new_y_2) / 2, 
                              label = line_id_1_2)) +
  geom_point(aes(x = star_new_x, y = star_new_y)) +
  geom_point(aes(x = constellation_new_x, y = constellation_new_y),
...
ggplot(data = sky) +
  ...
  coord_equal()
# Looks cool
# now needs to weight connections
# maybe add a couple more (probabilitic function of number of stars?)